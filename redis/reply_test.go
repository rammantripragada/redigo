...
var replyTests = []struct {
	...
	{
		"SlowLogs(1, 2, 3, {set, x, y}, localhost:1234, testClient",
		ve(getSlowLog()),
		ve(redis.SlowLog{ID: 1, UnixTime: 2, ExecutionTime: time.Duration(3) * time.Microsecond, Args: []string{"set", "x", "y"}, ClientAddr: "localhost:1234", ClientName: "testClient"}, nil),
	},
}

func getSlowLog() (redis.SlowLog, error) {
	slowLogs, _ := redis.SlowLogs([]interface{}{[]interface{}{int64(1), int64(2), int64(3), []interface{}{"set", "x", "y"}, "localhost:1234", "testClient"}}, nil)
	if err != nil {
		fmt.Println(err)
...
func TestSlowLog(t *testing.T) {
	...
	if err != nil {
		t.Errorf("TestSlowLog failed during redis.Strings with error " + err.Error())
		return
	}
