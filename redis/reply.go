...
func SlowLogs(result interface{}, err error) ([]*SlowLog, error) {
	...
	for i, rawLog := range rawLogs {
		...
		if err != nil {
			return nil, errors.New("redigo: slowlog element[3] is not array of string")
		}
		if len(rawLog) >= 6 {
			log.ClientAddr, err = String(rawLog[4], nil)
			if err != nil {
				return nil, errors.New("redigo: slowlog element[4] is not a string")
			}
			log.ClientName, err = String(rawLog[5], nil)
			if err != nil {
				return nil, errors.New("redigo: slowlog element[5] is not a string")
			}
		}
	}
	return logs, nil
